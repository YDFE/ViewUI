...
describe('DatePicker.vue', () => {
  ...
  it('should create a DatePicker component and open the calendar with the current month', done => {
    ...
    picker.$el.querySelector('input.ivu4-input').focus();
    vm.$nextTick(() => {
      const calendarBody = vm.$el.querySelector('.ivu4-picker-panel-body .ivu4-date-picker-cells:first-of-type');
      const calendarCells = [...calendarBody.querySelectorAll('.ivu4-date-picker-cells-cell')].filter(el => {
        const prevMonth = el.classList.contains('ivu4-date-picker-cells-cell-prev-month');
        const nextMonth = el.classList.contains('ivu4-date-picker-cells-cell-next-month');
  ...
  it('should create a datetimerange component and pick 2 dates in the current month', done => {
    ...
    vm.$nextTick(() => {
      const displayField = vm.$el.querySelector('.ivu4-input');
      const clickableCells = vm.$el.querySelectorAll('.ivu4-date-picker-cells-cell');
      const lastMonthClass = 'ivu4-date-picker-cells-cell-prev-month';
  ...
  it('should change type progamatically', done => {
    ...
    picker.handleFocus({type: 'focus'});
    vm.$nextTick(() => {
      const panel = vm.$el.querySelector('.ivu4-picker-panel-content');
      const dayPanel = panel.querySelector('[class="ivu4-date-picker-cells"]');
      const monthPanel = panel.querySelector('.ivu4-date-picker-cells-month');
      const yearPanel = panel.querySelector('.ivu4-date-picker-cells-year');
  ...
  it('should render date-picker label correctly in zh-CN', done => {
    ...
    vm.$nextTick(() => {
      const now = new Date();
      const labels = vm.$el.querySelectorAll('.ivu4-picker-panel-body .ivu4-date-picker-header-label');
